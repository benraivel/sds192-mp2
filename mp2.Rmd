...
finalSet %>%
  ...
  ggtitle("Average total positive donations to elections for incumbents vs challengers") +
  geom_col() +
...
finalNegSet %>%
  ...
  theme(panel.background = element_rect(fill = 'white'), legend.title = element_blank()) +
  ggtitle("Average total negative donations to elections for incumbents vs challengers") +
  geom_col()+
  scale_fill_manual(values = c("#a84521", "#20a866")) +
  facet_wrap("incumbent", labeller = labeller(incumbent = labels)) +
  scale_y_continuous(name = "Total negative contributions", breaks = c(0,-10000,-20000,-30000), labels = c("$0", "-$10,000", "-$20,000","-$30,000")) +
  scale_x_discrete(name = "", labels = c("Lost", "Won")) +
  guides(fill = FALSE)
```
